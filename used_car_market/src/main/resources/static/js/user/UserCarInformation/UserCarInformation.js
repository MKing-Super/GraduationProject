...
BootstrapTable.initTable = function () {
    $('#tb_departments').bootstrapTable({
        url: '/exhCar/userCarInformationList',         //请求后台的URL（*）
        method: 'get',                      //请求方式（*）
        ...
        columns: [                          //表头（注意区分大小写）
            ...
            //{field: 'collectiontime', title: '收藏时间', halign: 'center', align: 'center'},
...

            {
                ...
                formatter: function (value, row, index) {
                    var str = '<a class="btn btn-success" id="' + value + '" onclick="checkView(this)">查看</a>' +
                        '&nbsp;&nbsp;' +
                        '<a class="btn btn-warning" id="' + value + '" onclick="sold(this)">已售出</a>';
                    return str;
                }
            },
        ],
...
checkView = function (msg) {
    //跳转到新的页面
    window.open($("#RootPath").val() + "/exhCar/toUserCarInformation/" + msg.id);
};
 ...
 */
sold = function (msg) {
    if (confirm('您确定车辆已售出？') === true){
        //当前页面的跳转
        location.href = $("#RootPath").val() + "/exhCar/toUserCarSold/" + msg.id;
    }
};
